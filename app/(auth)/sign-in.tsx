...
export default function SignIn() {
  ...
  const submit = async ()=>{
    ...
    if (!form.email || !form.password) return Alert.alert("Error","Invalid Credentials")
      ...
      <CustomInput 
        ...
        <View className='flex justify-center mt-5 gap-2 flex-row'>
          <Text className='base-regular text-gray-100'>
            Don't have an account? <Link href="/(auth)/sign-up" className='base-bold text-primary'>Sign Up</Link>
          </Text>
        </View>
    </View>
  )
